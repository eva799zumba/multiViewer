...
class JpegWalkerTest {
    ...
    @Test
    fun `DQT decodes a 16-bit precision table`() {
        val bytes = byteArrayOf(
            0xff.toByte(), 0xd8.toByte(), 0xff.toByte(), 0xdb.toByte(), 0x00, 0x83.toByte(), 0x10, 0x03,
            0xe8.toByte(), 0x03, 0xe9.toByte(), 0x03, 0xf0.toByte(), 0x03, 0xf8.toByte(), 0x03,
            0xf1.toByte(), 0x03, 0xea.toByte(), 0x03, 0xeb.toByte(), 0x03, 0xf2.toByte(), 0x03,
            0xf9.toByte(), 0x04, 0x00, 0x04, 0x08, 0x04, 0x01, 0x03,
            0xfa.toByte(), 0x03, 0xf3.toByte(), 0x03, 0xec.toByte(), 0x03, 0xed.toByte(), 0x03,
            0xf4.toByte(), 0x03, 0xfb.toByte(), 0x04, 0x02, 0x04, 0x09, 0x04,
            0x10, 0x04, 0x18, 0x04, 0x11, 0x04, 0x0a, 0x04,
            0x03, 0x03, 0xfc.toByte(), 0x03, 0xf5.toByte(), 0x03, 0xee.toByte(), 0x03,
            0xef.toByte(), 0x03, 0xf6.toByte(), 0x03, 0xfd.toByte(), 0x04, 0x04, 0x04,
            0x0b, 0x04, 0x12, 0x04, 0x19, 0x04, 0x20, 0x04,
            0x21, 0x04, 0x1a, 0x04, 0x13, 0x04, 0x0c, 0x04,
            0x05, 0x03, 0xfe.toByte(), 0x03, 0xf7.toByte(), 0x03, 0xff.toByte(), 0x04,
            0x06, 0x04, 0x0d, 0x04, 0x14, 0x04, 0x1b, 0x04,
            0x22, 0x04, 0x23, 0x04, 0x1c, 0x04, 0x15, 0x04,
            0x0e, 0x04, 0x07, 0x04, 0x0f, 0x04, 0x16, 0x04,
            0x1d, 0x04, 0x24, 0x04, 0x25, 0x04, 0x1e, 0x04,
            0x17, 0x04, 0x1f, 0x04, 0x26, 0x04, 0x27, 0xff.toByte(),
            0xd9.toByte(),
        )
        val reader = byteReaderOf(bytes)
        val segments = parseJpegSegments(reader, 0, bytes.size.toLong())
        ...
        assertEquals(1, dqt.children.size)
        val table = dqt.children[0]
        assertEquals("1", table.fields.first { it.name == "precision" }.value)
        val expectedRaster = (1000..1063).map { it.toString() }
        assertEquals(GridData(8, 8, expectedRaster), table.grid)
        reader.close()
    }

    @Test
    fun `DQT quality estimate recovers a non-round-number quality from a scaled table`() {
        val bytes = byteArrayOf(
            0xff.toByte(), 0xd8.toByte(), 0xff.toByte(), 0xdb.toByte(), 0x00, 0x43, 0x00, 0x03,
            0x02, 0x02, 0x03, 0x02, 0x02, 0x03, 0x03, 0x03,
            0x03, 0x04, 0x03, 0x03, 0x04, 0x05, 0x08, 0x05,
            0x05, 0x04, 0x04, 0x05, 0x0a, 0x07, 0x07, 0x06,
            0x08, 0x0c, 0x0a, 0x0c, 0x0c, 0x0b, 0x0a, 0x0b,
            0x0b, 0x0d, 0x0e, 0x12, 0x10, 0x0d, 0x0e, 0x11,
            0x0e, 0x0b, 0x0b, 0x10, 0x16, 0x10, 0x11, 0x13,
            0x14, 0x15, 0x15, 0x15, 0x0c, 0x0f, 0x17, 0x18,
            0x16, 0x14, 0x18, 0x12, 0x14, 0x15, 0x14, 0xff.toByte(),
            0xd9.toByte(),
        )
        val reader = byteReaderOf(bytes)
        val segments = parseJpegSegments(reader, 0, bytes.size.toLong())
        ...
        val dqt = segments[1]
        val table = dqt.children[0]
        assertEquals("~90%", table.fields.first { it.name == "quality_estimate" }.value)
        reader.close()
    }
