...
class MetaBoxDecoderTest {
    ...
    @Test
    fun `corrupted XMP extent length is treated as out of bounds, not a crash`() {
        registerAllDecoders()
        val body = byteArrayOf(
            0x00, 0x00, 0x00, 0x5f, 0x6d, 0x65, 0x74, 0x61,
            0x00, 0x00, 0x00, 0x37, 0x69, 0x69, 0x6e, 0x66,
            ...
            0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x01, 0x00,
            0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
            0x00, 0x00, 0x5f, 0x00, 0x0f, 0x42, 0x3f, 0x3c,
            0x78, 0x3a, 0x78, 0x6d, 0x70, 0x6d, 0x65, 0x74,
            0x61, 0x3e, 0x74, 0x65, 0x73, 0x74, 0x3c, 0x2f,
            0x78, 0x3a, 0x78, 0x6d, 0x70, 0x6d, 0x65, 0x74,
            0x61, 0x3e,
        ...
        val ilocNode = node.children.first { it.type == "iloc" }
        val item1 = ilocNode.children.first { it.type == "item_1" }
        assertEquals(true, item1.fields.none { it.name == "xmp" })
        assertEquals(true, item1.warnings.isNotEmpty())
        reader.close()
    }

    @Test
    fun `corrupted Exif extent offset-length is treated as out of bounds, not a crash`() {
        registerAllDecoders()
        val body = byteArrayOf(
            0x00, 0x00, 0x00, 0x4b, 0x6d, 0x65, 0x74, 0x61,
            0x00, 0x00, 0x00, 0x23, 0x69, 0x69, 0x6e, 0x66,
        ...
        val ilocNode = node.children.first { it.type == "iloc" }
        val item1 = ilocNode.children.first { it.type == "item_1" }
        assertEquals(true, item1.children.none { it.type == "IFD0" })
        assertEquals(true, item1.warnings.isNotEmpty())
        reader.close()
    }
}
